...
- 신규 기능 추가가 끝나면 해당 패키지의 `info.yaml`도 함께 갱신한다.

## 패키지 기능 탐색 의무
- 모노레포에서 새로운 작업, 코드 개선, 기능 추가를 시작하기 전에 패키지들을 순회하며 각 패키지의 `info.yaml`을 먼저 탐색한다.
- 구현 방향/수정 대상 선정은 `info.yaml`에 기록된 `name: description` 기능 정보를 1차 기준으로 삼는다.
