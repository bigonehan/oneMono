# AGENTS.md

## 기능 추가/설계 전 사전 점검
- 기능 추가나 설계 전에 `packages/domain/` 또는 `packages/features/` 하위에 이미 같은 성격의 도메인/기능이 있는지 먼저 점검한다.
- 기존 구현이 있으면 우선 재사용/확장하고, 중복 생성은 피한다.

## 새 도메인 스키마 설계 규칙
- `packages/domains/` 아래에 새로운 도메인을 추가할 때는 구현 전에 먼저 `effect` 스킬을 사용해 도메인 스키마 초안을 설계한다.
- 새 도메인의 엔티티, 포트, 유스케이스, 어댑터 구조는 위 스키마 초안을 기준으로 정리한 뒤 파일 생성을 진행한다.

## UI 컴포넌트 처리 원칙
- UI 컴포넌트를 새로 만들기 전에 `packages/ui` 하위 패키지를 먼저 검사한다.
- 이미 있는 컴포넌트가 있으면 가급적 그것을 사용한다.
- 없으면 먼저 `packages/ui` 하위에 관련 라이브러리 기반 패키지를 만들고, 이후 앱에서 import 해서 사용한다.

## 신규 패키지 AGENTS.md 기본 규칙
- 새로운 패키지를 생성할 때는 해당 패키지 루트에 `AGENTS.md`를 기본으로 만든다.
- 신규 패키지의 기본 `AGENTS.md`에는 "기능 추가 또는 기능 개선이 있을 때 해당 패키지의 `package.json` 버전을 `0.0.1`씩 올린다"는 규칙을 포함한다.
- 별도 지시가 없으면 patch 단위 증가를 사용하고, 버전 세그먼트는 기본적으로 두 자리(`00`~`99`)를 넘기지 않도록 관리한다.

## Usecase 네이밍 규칙
- 도메인 usecase 파일명은 도메인 접두사 기반 snake_case를 사용한다.
- 파일명 패턴: `packages/domains/<domain>/src/usecases/<domain>_<action>.ts`
- 예시: `user_create.ts`, `user_get.ts`, `user_list.ts`, `user_update.ts`, `user_delete.ts`

## 폴더/파일 네이밍 규칙 
- 새 파일 생성 전 반드시 대상 도메인을 먼저 식별한다.
- 파일은 반드시 `domains/{entity명}/` 디렉토리 내부에 배치한다.
- 새로운 도메인이 필요하면 `domains/{entity명}/` 디렉토리를 새로 생성한다.
- 기본 구조:
- `domains/{entity명}/{entity명}_entit` (도메인 엔티티)
- `domains/{entity명}/{entity명}_port` (포트 인터페이스)
- `domains/{entity명}/{entity명}_service` (서비스, in adapter)
- `domains/{entity명}/{entity명}_{기술명}` (기술 어댑터, in/out)
- suffix 규칙:
- `_entity`: 도메인 엔티티
- `_port`: 포트 인터페이스
- `_service`: 서비스(in adapter)
- `_{기술명}`: 기술 어댑터(in/out) 예: `user_rest.py`, `user_pg.py`, `user_kafka.py`
- `_entity`, `_port`를 제외한 나머지 파일은 모두 어댑터로 간주한다.
- 어댑터 파일명에는 `_adapter` suffix를 붙이지 않고 기술명을 suffix로 사용한다.

예시 (`user` 도메인):

```text
domains/
└── user/
    ├── user_entity.ts
    ├── user_port.ts
    ├── user_service.ts
    ├── user_rest.ts
    └── user_pg.ts
```

## JJ Workspace 작업 규칙 (Mandatory)
- 구현/생성/개선 요청을 수행할 때는 코드 변경 전에 반드시 전용 `jj workspace`를 먼저 생성(또는 전환)한다.
- 코드 편집, 의존성 변경, 빌드/테스트 실행은 해당 작업용 `jj workspace`에서만 수행한다.
- 작업 완료 후 검증이 끝나면 기본 workspace로 변경사항을 반드시 merge 한다.
- merge 충돌 또는 merge 불가 상태가 발생하면 임의 우회하지 말고 즉시 사용자에게 확인한다.
- merge 완료 후에는 사용한 임시 workspace를 정리한다.
- 임시 workspace 정리는 반드시 두 단계로 수행한다.
- `jj workspace forget <workspace-name>`
- workspace 디렉터리 실제 삭제 (`rm -rf <workspace-path>` 또는 동등한 삭제 절차)
- 임시 workspace 정리 후 아래 후검증을 반드시 수행한다.
- `jj workspace list`에서 대상 workspace가 사라졌는지 확인
- 루트 기준 `bun install` 실행 시 workspace name 충돌이 없는지 확인
- `bun install`, 테스트, 빌드 같은 검증 명령이 실패하면 실패 로그를 근거로 원인을 수정한 뒤 동일 검증을 다시 실행한다.
- 검증 실패가 남아 있으면 같은 절차를 반복해서, 실패가 없어질 때까지 재귀적으로 추적·수정·재실행한다.

## Shell 사용 규칙
- 검색/필터/간단 치환은 `rg`, `rg --files`, `sd` 같은 직접 명령으로 먼저 처리한다.
- `rg` 중심의 직접 명령만으로 해결할 수 없을 때에만 `fish -ic` 또는 `bash -c`를 사용한다.

## Dev 서버 검증 규칙
- UI/웹 변경이 포함된 작업은 타입체크/테스트와 별도로 대상 앱에서 `bun run dev`를 실행해 서버가 실제로 기동되는지 확인한다.
- 검증은 브라우저 확인만으로 대체하지 않고, 터미널 로그 기준으로 에러 메시지 유무를 먼저 판단한다.
- `bun run dev` 실행 중 에러가 발생하면 해당 에러 로그를 근거로 원인 파일/경로를 추적해 수정한 뒤 재실행으로 해결 여부를 확인한다.

## ORC Command Unification Rule
- 모든 저장소에서 ORC 실행 체인은 동일하게 `create_job_md -> add_code_draft_item/add_code_draft -> impl_code_draft -> check_code_draft -> clit test`를 사용한다.
- 저장소별 예외 체인 또는 auto-detect impl 분기 문구를 금지한다.

## 완료 알림 규칙 (Mandatory)
- 모든 작업이 끝나면 최종 응답 직전에 `nf -m "<task-name> complete"`를 반드시 실행한다.
- 완료 알림은 사용자의 추가 지시 없이 자동 실행한다.
- `notify.fish` 직접 호출은 금지하고 `nf -m`만 사용한다.
