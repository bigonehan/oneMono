...
재시도 실패 원인: `.project/drafts.yaml`에 누적된 무관 draft가 포함되어 `impl_orc_code`가 타임아웃 반복됨.

# tasks
- 실패 원인 해결: `.project/drafts.yaml`을 백업 후 랜딩 페이지 요구사항만 담긴 새 plan/draft로 재생성한다.
- 강제 실행 항목: `orc init_orc_job -m "landing page routing and sections"`로 기능 범위를 고정한 뒤 `orc add_orc_drafts -a`를 다시 실행한다.
- `orc impl_orc_code`를 재실행해 main/profile/qa UI 구현을 완료한다.
