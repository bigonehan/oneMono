...
- ORC workflow로 `init_code_project -> init_code_plan -> create_input_md -> add_code_draft -> impl_code_draft -> check` 순서를 실제 실행한다.
- 실패 원인 해결: `create_input_md` 정체와 generic draft를 우회하기 위해 `input.md`를 수동으로 구체화하고 `add_code_draft -f`로 재생성한다.
- 강제 실행: `input.md`에는 소개/detail/progress CTA/Stripe webhook/admin editor/data persistence를 모두 명시한다.
- 실패 원인 해결: `impl_code_draft` 비수렴을 우회하기 위해 Next 패키지 파일과 서버/API 흐름을 직접 구현한다.
...
- 실제 검증 보강: dev 서버를 띄운 뒤 admin 저장 API와 landing 렌더 반영을 HTTP 요청으로 확인하고 결과를 `feedback.md`에 누적한다.
- `rg`로 `UI 입력 -> 핸들러 -> 내부 로직 -> 상태/파일 변경 -> 화면 반영` 호출 경로를 점검한다.
- `npm run lint`, `npm run check-types`, `npm run build`, `rc` 체크, 필요 시 재시도 후 `feedback.md`에 보완점을 누적한다.
...
- 재시도 사유: `create_input_md` hang -> `input.md` 수동 작성 후 `add_code_draft -f` 재실행
- 재시도 사유: `impl_code_draft` non-converging -> 수동 구현 후 전체 검증 재실행
