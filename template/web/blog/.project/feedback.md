...
# 해결
- 기존 `.project` 이력 때문에 `orc auto -> create_input_md`와 초기 `add_code_draft -m`이 현재 스폰서 작업 대신 이전 draft를 잡던 문제를 `plan.yaml`/`input.md` 정리와 `add_code_draft -f` 재진입으로 해소했다.
- `orc impl_code_draft`가 소스 변경 없이 LLM wait만 반복하던 문제는 직접 구현으로 전환해 실제 페이지/스타일/테스트 변경을 완료했다.
...
#개선필요
- 기존 `.project` 이력이 남아 있는 템플릿에서는 `orc auto` 전체 루프보다 기능 범위를 좁힌 `add_code_draft -f` 재진입 경로를 먼저 점검한다.
