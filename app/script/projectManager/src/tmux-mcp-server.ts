...
import { ensureManagerPaneId } from "./tmux.ts";
import { logTaskStart, markTaskComplete } from "./grist-task-logger.ts";
import { CliState, loadState, writeState } from "./grist.ts";
  ...
  try {
    ...
    switch (name) {
      ...
      case "one_coding": {
        ...
        const planPath = isAbsolute(planArg) ? planArg : resolve(workspaceArg, planArg);

        if (!existsSync(planPath)) {
          throw new Error(`plan 파일을 찾을 수 없습니다: ${planPath}`);
        }
